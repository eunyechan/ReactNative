...
export default class MainComponent extends Component{
    ...
    render(){
        return(
            ...
            //1)글씨를 입력받을 수 있는 기본적인 TextInput사용
            ...
            //방법이 기존과 많이 다름.. --컴포넌트객체를 참조하여 제어하는 방식이 아닌..binding방식으로 데이터 보여줌
            ...
            //     {/* Text컴포넌트가 보여줄 내용물이 고정된 글씨가 아니라 바뀌게 되는 값이어서 변수로 글시 설정: 버튼예제에서 해봤듯이 컴포넌트가 보여줄 값은 state객체를 이용하기 */}
            //     {/* state의 값이 변경되면 자동 render()메소드가 다시 실행되어 자연스럽게 갱신됨 */}
            //     <Text style={style.plainText}>{this.state.text}</Text>
            // </View>
            ...
            //네이티브앱개발 방식을 버릴것!
            //TextInput에 글씨가 변경될 때마다 변수에 값을 저장하고..버튼을 클릭할 때 Text컴포넌트가 보여주는 state에 값 설정!!
            //즉, Text컴포넌트가 보여주는 멤버변수와 TextInput컴포넌트가 수정할 멤버변수를 다른 변수로 사용
            // <View style={style.root}>
            //     {/* multiline속성은 boolean값을 원하므로 JS로 값을 주어야함 */}
    ...
    //2)실습 onChangeText속성에 의해 실행될 멤버메소드 [this키워드로 state변수를 사용하려면 화살표함수로..일반함수를 사용하려면 bind()써야함--Ex02ButtonEvent예제에서 소개..]
    // changeText=()=>{
    //     Alert.alert('change TEXT'); //호출되는 것 확인
    // }
    //참고로 Alert을 change될때마다 띄우기 때문에 앱을 다시 실행시키는 'RR' 키보드를 누를 때마다 다이얼로그가 띄워져서 리로드가 번거로우니..
    //ctrl+M(WindowOS)/cmd+M(MacOS)를 눌러 디버그 메뉴 통해 리로드하면 수월함.

    //TextInput의 글씨가 변경될 때마다 Text가 보여주는 값 this.state.text를 변경하기
    //사실 onChangeText속성은 설정한 함수를 실행할 때 파라미터(매개변수)로 변경되고 있는 글씨를 전달해줌.. 이를 받으려면 파라미터에 변수 선언
    changeText=(value)=>{
    ...
    submitText=(submitEvent)=>{
        //전달되어온 파라미터가 다름. changeText와 달리 변경된 값이 오는 것이 아니라. submit이벤트 객체가 옴.
        //그래서 이 이벤트 객체로부터 값을 얻어와야함.
