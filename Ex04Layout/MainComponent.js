...
export default class MainComponent extends Component{
    ...
    render(){
        return(
            //여러개의 뷰컴포넌트를 넣으려면 일단 가장 큰 뷰가 있어야함. why? return은 1개의 컴포넌트만 리턴할 수 있으므로.
            ...
            //     {/* width, height에 지정하는 사이즈 값으로는 숫자와 %를 사용할 수 있음. */}
            //     {/* 숫자는 단위가 dp임.[px를 명시적으로 쓰지 못함], %쓸때는 문자열' '로 지정해야함 */}
            //     {/* 별도로 부모뷰에 스타일을 주지 않으면 기본적인 자식뷰들은 position이 relative레이아웃 배치로 보이지만 flex임 */}
            ...
            //     {/* android의 layout_weight과 흡사한 기능: 현재 수직배치이므로 높이값을 설정하느 것임 */}
            //     <View style={{flex:1, backgroundColor:'red'}}></View>
            //     <View style={{flex:2, backgroundColor:'green'}}></View>
            //     <View style={{flex:4, backgroundColor:'blue'}}></View>
            //     {/* 부모뷰의 높이가 지정되어 있지 않으면 자식뷰의 flex는 높이를 계산하지 못하여 화면에 출력되지 못함 */}
            //     {/* 즉, 자식뷰의 너비나 높이를 flex를 이용하려면 부모뷰의 너비나 높이가 반드시 있어야함 */}
            //     {/* 우선 부모뷰에 height값으로 350을 지정해보기[기본 담위는 dp임] */}
            //     {/* 부모뷰의 높이 350을 1:2:4의 비율로 차지하게됨. */}
            ...
            //     {/* 반대로 높이는 무조건 stretch되어 march_parent가 됨 */}
            // </View>

            //5)자식뷰들의 높이와 너비를 개별 지정하여 배치 정렬해보기
            //css의 flex justify-content와 align-items와 같은 방식
            //space-evenly: 모든 여백이 같은 크기, space-around: 각 요소를 둘러싼 여백의 크기가 같게, space-between: 각 요소들 사이의 여백이 같게.
            ...
            <View style={{flex:1, flexDirection:'column'}}>
                ...
                <View style={{flex:1, flexDirection:'row'}}>
                    ...
                    <View style={{flex:2, backgroundColor:'red'}}>
                        ...
                        {/* 절대 위치 지정으로 뷰 겹치기 : 절대위치설젖ㅇ을 하지 않으면 겹치지 않음. */}
                        <View style={{width:50, height:50, backgroundColor:'white', left:10, top:10, position:'absolute'}}></View>
                        <View style={{width:50, height:50, backgroundColor:'gray', left:10, top:10, position:'absolute'}}></View>
...
//index.js에서 import를 할 수 있으려면 이 문서에서 export해야함
